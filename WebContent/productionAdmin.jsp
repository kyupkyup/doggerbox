	...
	<button onclick="location.href='roundInsert.jsp'">차수 추가</button>
		...
		%>
			<tr>
				...
				<td><button onclick="location.href='calculateIngredients.jsp?sumPuppy=<%=sumPuppy%>&sumOriginal=<%=sumOriginal%>&sumSenior=<%=sumSenior%>&sumFish=<%=sumFish%>&sumPork=<%=sumPork%>&sumKangaroo=<%=sumKangaroo%>&sumHorse=<%=sumHorse%>'">개별 재료량 산출</button></td>

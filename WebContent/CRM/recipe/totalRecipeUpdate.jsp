	...
	%>
		...
		<form action="totalRecipeUpdateAction.jsp?ingredientPrimeNum=<%=ingredientPrimeNum%>" method="post">
			...
			<div>
				...
				<input type="text" class="form-control" placeholder="손실율(소수 형식 ex. 0.2)" name="lossRate" maxlength="50" value="<%=ingredientDAO.getIngredientLossRate(ingredientPrimeNum)%>"></input>
			</div>
			<div>
				<p>원가(kg당)</p>
				<input type="text" class="form-control" placeholder="원가(숫자만)" name="ingredientPrice" maxlength="50" value="<%=ingredientDAO.getIngredientPrice(ingredientPrimeNum)%>"></input>
			</div>
